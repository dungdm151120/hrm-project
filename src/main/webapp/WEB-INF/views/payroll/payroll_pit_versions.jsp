    ...
    <div class="dashboard-main">
        <div class="dashboard-header">
            <div class="header-left">
                <h1 class="header-title">Payroll PIT Brackets Versions</h1>
            </div>
            <div class="header-right">
                <a href="${pageContext.request.contextPath}/payroll/pit/update" class="btn-save">
                    + Create New Version
                </a>
            </div>
        </div>
        ...
        <div class="dashboard-content">
            ...
            <div class="filter-card">
                <form action="${pageContext.request.contextPath}/payroll/pit/list" method="GET" class="filter-form">
                    ...
                    <c:if test="${not empty month || not empty year}">
                        <a href="${pageContext.request.contextPath}/payroll/pit/list" class="btn-reset" style="font-size: 0.82rem;">
                            Clear Filters
                        </a>
                    </c:if>
                </form>
                        ...
                        <c:forEach var="version" items="${versions}" varStatus="loop">
                            <tr>
                                <td style="text-align: center; font-size: 0.85rem;">
                                    ${(currentPage > 0 ? (currentPage - 1) * 10 : 0) + loop.index + 1}
                                </td>
                                <td style="text-align: center;">
                                    <strong style="color: #1e293b; font-size: 0.88rem;">${version.versionName}</strong>
                                </td>
                                <td>
                                    <strong style="color: #334155; font-size: 0.88rem;">${version.effectiveDate}</strong>
                                </td>
                                <td style="text-align: center;">
                                    <c:choose>
                                        <c:when test="${not empty today && version.effectiveDate > today}">
                                            <span class="badge-status badge-upcoming-status">UPCOMING</span>
                                        </c:when>

                                        <c:when test="${version.id == activeId || version.id == latestId}">
                                ...
                                <td style="text-align: center;">
                                    <div style="display: flex; gap: 6px; justify-content: center; align-items: center;">
                                        <a href="${pageContext.request.contextPath}/payroll/pit?id=${version.id}&effectiveDate=${version.effectiveDate}"
                                           class="btn-save"
                                           style="padding: 4px 10px; font-size: 0.78rem;">
                                            View Details
                                        </a>

                                        <form action="${pageContext.request.contextPath}/payroll/pit/delete" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this PIT version?');">
                        ...
                        <c:if test="${empty versions}">
                            <tr>
                                <td colspan="5" class="empty-state" style="text-align: center; padding: 32px; color: #64748b;">
                                    No PIT version records found for the selected criteria.
                                </td>
                            </tr>
                        </c:if>
                    </tbody>
                ...
                <c:if test="${not empty totalPages}">
                    <div class="pagination-wrapper">
                        ...
                        <div class="pagination-nav">
                            <a href="${pageContext.request.contextPath}/payroll/pit/list?page=${currentPage - 1}&month=${month}&year=${year}"
                               class="page-link ${currentPage == 1 ? 'disabled' : ''}">
                                ‹ Prev
                            </a>

                            <c:forEach var="p" begin="1" end="${totalPages}">
                                <a href="${pageContext.request.contextPath}/payroll/pit/list?page=${p}&month=${month}&year=${year}"
