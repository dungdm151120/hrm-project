...
-- ==========================================
-- 2. ATTENDANCE & LEAVE TABLES (From DB1)
-- ==========================================
...
CREATE TABLE leave_balances (
    ...
    user_id INT NOT NULL,
    year INT NOT NULL,
    entitled_days DECIMAL(5,2) DEFAULT 12.00 COMMENT 'Số ngày phép được cấp trong năm',
    advanced_days DECIMAL(5,2) DEFAULT 0.00 COMMENT 'Số ngày phép được ứng thêm',
    used_days DECIMAL(5,2) DEFAULT 0.00 COMMENT 'Số ngày phép đã sử dụng',
    remaining_days DECIMAL(5,2) DEFAULT 12.00 COMMENT 'Số ngày phép còn lại',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NULL ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_leave_balances_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT unique_leave_balance_user_year UNIQUE (user_id, year)
                                 id INT PRIMARY KEY AUTO_INCREMENT,
                                 work_date DATE NOT NULL,
                                 employee_id INT NOT NULL,
                                 check_in DATETIME NULL,
...
-- ==========================================
-- 3. REQUEST TABLES (Added from DB2)
-- ==========================================
...
-- Sử dụng danh sách user từ DB2 (Cấu trúc gọn hơn, gộp System Admin vào EMP001)
INSERT INTO users (
